...
Берёт первого овнера из /dev_auth/users, шлёт POST /apartments/parse-url,
печатает ответ.
"""
import json
import os
...
def pick_owner_id() -> int:
    ...
    for u in r.json():
        if u["role"] == "owner":
            return u["id"]
    raise SystemExit("в базе нет пользователя с ролью owner — seed его руками")
...
**Замечание:** этот e2e требует (а) запущенного бэка (`./start.sh` или `uv run --env-file .env uvicorn backend.main:app --port 8000`), (б) `DEBUG=1` (иначе `/dev_auth` не зарегистрирован), (в) хотя бы одного юзера с `role=owner` в БД.
