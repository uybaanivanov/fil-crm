...
from backend.morelogin import start_profile, stop_profile
...
```python
"""Полный пайплайн: URL → MoreLogin → Playwright → парсер. Интерактивно."""
import asyncio
import json

from backend.routes.apartments import _fetch_and_parse


async def main():
    url = input("URL: ").strip()
    listing = await _fetch_and_parse(url)
